...
- if we run simulations of fitting the model predicting kilocalories per gram with percent fat and a new variable that is correlated wih percent fat at different amounts, we can plot the standard deviation of the coefficient of percent fat against the correlation of the two predictors
...
- summary of `m5_13`:
    * the coefficent for treatment is basically 0 with the 89% intervals straddling 0
    * the coefficients for starting height and precense of fungus were very strong
...
- we can get posteior distribution for male height by sampling $\alpha$ and $\beta_m$ from the model and adding them together
    * we cannot just add the 89% interval values togehter because $\alpha$ and $\beta_m$ are correlated
...
- *ordinary least squares*: (OLS) way of estimating the parameters of linear regression
    ...
    * generally simillar to maximizing the posterior probability or maximizing the likelihood
...
- categorical variables are automatically expanded into the necessary dummy variables
    * if the cetagorical variable is encoded with numbers, you must change it to a factor (or character)
...
I got my statisitcs from the *Wikipedia* article ["The Church of Jesus Christ of Latter-day Saints membership statistics (United States)"](https://en.wikipedia.org/wiki/The_Church_of_Jesus_Christ_of_Latter-day_Saints_membership_statistics_(United_States)) (05/06/2020).
...
I would begin by modeling the obesity rate on just the about of exercise and number of restaurant meals per month (or some other period) to see if they directly predict obestiy.
Further, I would model obesity with both of these predictors as they could be negatively correlated with each other, casuing a masked relationship.
I would then look at the counterfactual plots and residulas to see if either predictor is more influential than the other.

### Hard
**All three exercises below use the same data, data(foxes) (partof rethinking).81 Theurban fox (Vulpes vulpes) is a successful exploiter of human habitat. Since urban foxes move in packs and defend territories, data on habitat quality and population density is also included. The data frame has five columns:**
